...
export default function TaskTimer({
  ...
  const updateTaskTime = async (taskId, sessionTime) => {
    try {
      ...
      const end = new Date(Date.now() + sessionTime * 1000).toISOString();
      ...
      const response = await fetch("/api/update-time", {
        ...
        body: JSON.stringify({ taskId, start, end, sessionTime }),
      });
  ...
  return (
    ...
    >
      <CountdownCircleTimer
        ...
        onComplete={() => {
          audio.play();
          updateTaskTime(selectedTaskId, duration ); // Convert seconds to minutes
          onComplete();
